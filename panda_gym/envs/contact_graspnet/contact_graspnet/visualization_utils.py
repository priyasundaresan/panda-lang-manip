...
#        index += N
#        #pts = np.vstack([pts[4], pts[5]])
#        #print('HERE', np.linalg.norm(np.array(pts[5]) - np.array(pts[4])))
...
def interp(p1, p2):
    ...
    offset = p2 - p1
    num_points = 30
    delta = offset/num_points
    #delta_is = np.arange(num_points).reshape(-1,1)
    #print(delta_is.T.shape, delta.shape)
    points = np.array([p1 + i*delta for i in range(num_points)])
    #points = (delta_is.T*delta) + p1
...
def get_grasps(grasps, cam_pose, gripper_openings):
    ...
    for i,(g,g_opening) in enumerate(zip(grasps, gripper_openings)):
        ...
        pts_grasp_center = np.vstack([pts[4], pts[5]])
