...
class TableTop:
    def __init__(
        ...
        #self.primitive_inference_server = Inference(checkpoint_path='checkpoints_prim/log/part_seg/2023-03-26_23-01', inp_dim=7)
        self.primitive_inference_server = Inference(checkpoint_path='checkpoints_prim/log', inp_dim=7)
    ...
    def RandX(self,N, r_threshold):
        ...
        while len(points) < N:
            ...
            new_point = np.random.uniform([-0.3,-0.3,0.075], [0.0,0.0,0.075])
    ...
    def reset_sim(self, num_locs=3):
        ...
        filtered_points = self.RandX(num_locs, 0.09)
    ...
    def take_rgbd(self, prompt):
        ...
        points = points[idxs]
        colors = colors[idxs]
        idxs = np.where(points[:,2] > 0.002)[0]
        points = points[idxs]
        colors = colors[idxs]
        ####
    ...
    def execute(self, episode_idx, instr, action):
        ...
        points = (rot@pcl_points.T).T
        pcd.points = o3d.utility.Vector3dVector(points)
        pcd.colors = o3d.utility.Vector3dVector(pcl_colors/255.)
